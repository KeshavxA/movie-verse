...
import { ArrowLeft, Star, Clock, Calendar, DollarSign, Heart, MessageSquare, Share2, Check, Twitter, MessageCircle } from "lucide-react";
...
const MovieDetails = () => {
  ...
  const handleTwitterShare = () => {
    ...
    const text = encodeURIComponent(`Check out ${movie?.title} on MovieVerse!`);
    window.open(`https://twitter.com/intent/tweet?text=${text}&url=${url}`, '_blank');
  };

  const handleWhatsappShare = () => {
    const url = encodeURIComponent(window.location.href);
    const text = encodeURIComponent(`Check out ${movie?.title} on MovieVerse! ${url}`);
