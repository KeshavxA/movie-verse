...
const VideoGallery = ({ videos }) => {
  ...
  useEffect(() => {
    if (videos && videos.length > 0) {
      // Try to find an official trailer first, otherwise just use the first video
        ...
        <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-thin scrollbar-thumb-slate-300 dark:scrollbar-thumb-slate-700 scrollbar-track-transparent">
          {videos.map((video) => (
            <button
              ...
              className={`flex-shrink-0 w-48 relative group rounded-xl overflow-hidden border-2 transition-all duration-300 ${
